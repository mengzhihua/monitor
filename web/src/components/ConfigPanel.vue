...
async function saveLocal() {
  if (!props.canManage || busy.value || !local.value) return
...
async function restart() {
  ...
  if (!confirm('重启将短暂中断本机监控与 Web 服务（数秒），由 systemd 自动拉起。确定继续？')) return
  busy.value = true; error.value = ''; message.value = ''
  try {
...
<template>
  <section class="config-panel" aria-label="配置管理">
    ...
    <div v-if="tab === 'local'">
      ...
      <template v-if="local">
        <p class="meta">文件：<code>{{ local.path }}</code> 修改于 {{ time(local.updated) }}</p>
        <textarea v-model="localDraft" aria-label="本机配置内容" rows="18" spellcheck="false" :readonly="!canManage" class="mono"></textarea>
        <p v-if="!canManage" class="dim">只有管理员可以编辑配置。</p>
        <div class="row">
          <button :disabled="!canManage || busy || loading" @click="saveLocal">保存本机配置</button>
