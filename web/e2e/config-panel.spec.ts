...
test('restart posts only after confirmation', async ({ page }) => {
  ...
  await button.click()
  expect(messages[0]).toContain('服务将自动重新启动')
...
test('non-admin cannot open the credential-bearing config editor', async ({ page }) => {
  const f = await fixture(page, { role: 'viewer' })
  await page.goto('/?view=charts&token=browser-viewer-token')
