...
func TestManageConfigGetPut(t *testing.T) {
	...
	// GET returns the file text, its size and a microsecond mtime revision.
	resp, err := http.Get(ts.URL + "/api/v1/manage/config?token=admin-token")
	if err != nil {
		t.Fatal(err)
	}
	...
	if got.Path != path || got.YAML != initial || got.Updated != info.ModTime().UnixMicro() || got.Size != len(initial) {
		t.Fatalf("get = %+v", got)
	}
	...
	put := func(body map[string]any) *http.Response {
		...
		req, _ := http.NewRequest(http.MethodPut, ts.URL+"/api/v1/manage/config?token=admin-token", bytes.NewReader(b))
	...
	st, _ := os.Stat(path)
	if st.ModTime().UnixMicro() == 0 {
		t.Fatal("no mtime after put")
	}

	// stale if_updated -> 409 without touching the file
	if resp := put(map[string]any{"yaml": next, "if_updated": st.ModTime().UnixMicro() + 9999}); resp.StatusCode != http.StatusConflict {
	...
	if resp := put(map[string]any{"yaml": next, "if_updated": st.ModTime().UnixMicro()}); resp.StatusCode != http.StatusOK {
...
func TestManageConfigMissingFile(t *testing.T) {
	ts, _ := newTestServer(t, Options{Token: "admin-token", ConfigPath: filepath.Join(t.TempDir(), "monitor.yaml")})
	resp, err := http.Get(ts.URL + "/api/v1/manage/config?token=admin-token")
	if err != nil {
		t.Fatal(err)
	}
...
func TestManageRestart(t *testing.T) {
	var calls int32
	ts, _ := newTestServer(t, Options{Token: "admin-token", RequestRestart: func() error { atomic.AddInt32(&calls, 1); return nil }})
	resp, err := http.Post(ts.URL+"/api/v1/manage/restart?token=admin-token", "application/json", nil)
	if err != nil {
		t.Fatal(err)
	}
	var out struct {
	...
	resp.Body.Close()
	if resp.StatusCode != http.StatusAccepted || !out.OK {
...
func TestManageRestartUnsupported(t *testing.T) {
	ts, _ := newTestServer(t, Options{Token: "admin-token"})
	resp, err := http.Post(ts.URL+"/api/v1/manage/restart?token=admin-token", "application/json", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestManageConfigAdminOnly(t *testing.T) {
	...
	ts, _ := newTestServer(t, Options{Token: "admin-token", ConfigPath: path, Users: []User{
