...
func (s *Store) Apply(id, actor string, change Change, revision uint64, target Target) (Record, error) {
	...
	a := Action{At: time.Now().Unix(), Actor: actor, Action: change.Action, Note: strings.TrimSpace(change.Note)}
	...
	next := state{Version: 2, Records: make(map[string]Record, len(s.state.Records)+1)}
