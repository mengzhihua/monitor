...
| **M23 IBM 与残留应用（本轮）** | ibm.d db2/as400/mq/websphere（CLI/HTTP，无 CGO）；pandas/go_expvar/am2320；lxc/ecs/containerd | 长尾，不挡主路径 | 无驱动/无 socket 禁用 |
...
与 M19–M22 并行。默认无 CGO；Init 失败即禁用。
