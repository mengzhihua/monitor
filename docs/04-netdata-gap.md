...
| **M12 续 5（本轮）** | ap, dockerhub, ethtool, intelgpu, logind, dcgm, panos, powerstore, powervault, s3check, scaleio, smbios_memory |
| **M12 续 6** | snmp_traps, snmp_topology, cloudwatch, azure_monitor, vsphere, mssql, oracledb, sql, vcsa, cato_networks |
...
| **M12 续 5（本轮）** | ap/dockerhub/ethtool/intelgpu/logind/dcgm/panos/powerstore/powervault/s3check/scaleio/smbios_memory | 第六批长尾 12 个 |
| **M12 续 6** | snmp_traps/topology、云厂商、mssql/oracledb/sql、vcsa/cato | 清单打勾 |
...
1. 采集器：`ap` `dockerhub` `ethtool` `intelgpu` `logind` `dcgm` `panos` `powerstore` `powervault` `s3check` `scaleio` `smbios_memory`  
2. CLI / HTTP / REST 探测，目标缺失自动禁用  
3. 内置规则：光模块温度、Intel GPU busy、DCGM GPU 温度、PAN-OS session、PowerStore/PowerVault health、S3 check、ScaleIO capacity
