...
| **M12 续** | snmp_traps, snmp_topology, cloudwatch, azure_monitor, vsphere, redfish, megacli, hpssa, adaptec_raid, 以及 init.go 其余模块 |
...
| **M12 续** | snmp_traps/topology、云厂商、RAID、init.go 其余 | 清单打勾 |
...
1. 采集器：`snmp` `proxysql` `clickhouse` `cockroachdb` `pulsar` `envoy` `upsd` `zfspool` `dmcache` `filecheck` `supervisord` `monit`  
2. 默认端口 / CLI 探测，目标缺失自动禁用（`filecheck` 无路径则禁用）  
3. 内置规则：ZFS degraded、NUT 电池、ProxySQL slow、Envoy 5xx、文件缺失、Supervisord/Monit、SNMP ifDown、Cockroach live nodes
