...
| 层 | 选型 | 理由 |
| --- | --- | --- |
| 服务端核心（Agent / Hub） | **Go 1.22+**，单静态二进制 | 一份代码交叉编译 linux/darwin/windows/android(arm64,arm,amd64)；`gopsutil` 覆盖四平台系统指标；goroutine 适合上千采集器并发；`gomobile bind` 可编成 Android AAR。 |
...
| **M15 ML / Functions / 导出（本轮）** | k-means、containers/disks/mounts/ifaces、Mongo 导出 | weights 对比 |
| **M16** | Windows/FreeBSD、Flutter、Android Agent | 平台与客户端 |
