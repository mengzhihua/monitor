...
后续：M19 内核深度（eBPF/perf）→ M20 日志/查看器 → M21 Windows.plugin → M22 freebsd.plugin → M23 IBM/残留 → M24 API v3 → M25 Cloud 产品面 → **M26 点名 Prometheus 原生 ID（本轮）**。详见 [docs/04-netdata-gap.md](docs/04-netdata-gap.md) 与架构文档 §11。
